 ...
 * 
 * Example use of secondary sort and a sliding window to produce a moving average.
 * 
 * As opposed to loading all of the points into a data structure beforehand, this Reducer loads only as many points
 * as are needed to fill the window, continually streaming the points through the window as it receives them.
 * 
 * 
 * Ignores the fact that values may be missing, calculates window based on time delta as opposed to number of samples/points in window.
 * 
 * When only stepping one day we could get away with a simpler algorithm that was more efficient,
 * but this example is meant to show how a full sliding window would work.
 * 
 * Also notice the copying of the points into the sliding window; this is because Hadoop reusues Writables.
 * 
 * 
 * @author jpatterson
 *
 */
public class MovingAverageReducer  extends MapReduceBase implements Reducer<TimeseriesKey, TimeseriesDataPoint, Text, Text> {
    ...
    public void configure(JobConf job) {
	 ...
	 public void reduce(TimeseriesKey key, Iterator<TimeseriesDataPoint> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
	    	...
	    	int iWindowSizeInDays = this.configuration.getInt("tv.floe.examples.mr.sax.windowSize", 30 );
	    	int iWindowStepSizeInDays = this.configuration.getInt("tv.floe.examples.mr.sax.windowStepSize", 1 );
			...
			out_key.set( "debug > " + key.getGroup() + " --------- end of group -------------"  );
			out_val.set( "" );
