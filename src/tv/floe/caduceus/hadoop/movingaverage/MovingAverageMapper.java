 ...
 * MovingAverageMapper
 * 
 * The primary purpose of this map class is to read each line from the CSV file split, parse out
 * the timeseries point record with the YahooStockDataPoint class, and emit a timeseries k/v pair
 * 
 * In this case we're emitting custom key and value pairs for this example in the TimeseriesKey ( key / WriteableComparable ) and TimeseriesDataPoint ( value / Writeable )
 * 
 * @author jpatterson
 *
 */
public class MovingAverageMapper  extends MapReduceBase implements Mapper<LongWritable, Text, TimeseriesKey, TimeseriesDataPoint> 
{
	...
	static enum Timeseries_Counters { BAD_PARSE, BAD_LOOKUP };
	   ...
	   private static final Logger logger = Logger.getLogger( MovingAverageMapper.class );
